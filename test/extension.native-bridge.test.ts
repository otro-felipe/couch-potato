...
describe("native extension bridge", () => {
  it("uses the fixed native host and emits correlated versioned responses", async () => {
    ...
    bridge.start();
    port.messageListener?.({ protocol: "1", id: "status-1", method: "bridge.status", params: {} });
    await flush();

    expect(runtime.connectNative).toHaveBeenCalledWith("com.couch_potato.browser_bridge");
    expect(port.responses).toEqual([
      { protocol: "1", id: "status-1", ok: true, result: { attachedTabIds: [] } },
    ]);
    bridge.stop();
  ...
  it("never reflects invalid inputs or internal errors", async () => {
    ...
    port.messageListener?.({ protocol: "1", id: "one", method: "bridge.status", params: {} });
    port.messageListener?.({ protocol: "1", id: "two", method: "bridge.status", params: {} });
    await flush();

    expect(port.responses).toEqual([
      { protocol: "1", id: "invalid-request", ok: false, error: { code: "INVALID_REQUEST" } },
      { protocol: "1", id: "one", ok: false, error: { code: "TIMEOUT" } },
      { protocol: "1", id: "two", ok: false, error: { code: "INTERNAL_ERROR" } },
    ]);
  });
  ...
  it("reconnects once after failure or disconnection and ignores stale ports", () => {
    ...
    const runtime: NativeRuntime = {
      connectNative: vi.fn().mockImplementationOnce(() => {
