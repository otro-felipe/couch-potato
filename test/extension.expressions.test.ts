...
import { locatorObjectExpression, locatorProbeExpression } from "../src/extension/locator-expression.js";
...
describe("locator expressions", () => {
  it("builds CSS, text, and role resolvers without embedding raw tag delimiters", () => {
    ...
    const text = locatorProbeExpression({ type: "text", value: "Continue", exact: true });
    const role = locatorProbeExpression({ type: "role", role: "button", name: "Continue", exact: false });
...
describe("safe bridge faults", () => {
  it("preserves allowlisted faults and maps unknown failures", () => {
    ...
    expect(asBridgeFault(new Error("private"))).toMatchObject({ code: "INTERNAL_ERROR" });
